...
export async function getStaticProps(context) {
  ...
  const query = gql`
    query {
      pageConnection(
        sort: [url__asc]
        first: 100
        query: { term: { parentUrl: { value: "${endPoint}" } } }
        aggs: {
          key: "facets"
          value: {
  ...
  `;
  const page = (await client.query({ query: query })).data.pageConnection;
  const slugType = page?.edges[0]?.node?._source?.type;
  const count = page.count;
  const output = page?.edges.map(({ node }) => {
    return {
      title: node._source.title,
      desc: node._source.description,
      url: node._source.url,
      parentUrl: node._source.parentUrl,
